...
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) {
  ...
  for (int i = 0; i < height; i++){
    ...
    DMA[3].cnt = width | DMA_ON | DMA_DESTINATION_INCREMENT | DMA_SOURCE_FIXED | DMA_16 ;
  }
  //DMA[DMA_CHANNEL_3].cnt = 0; //turn off any previous drawing
}
...
void drawFullScreenImageDMA(const u16 *image) {
  ...
  DMA[3].dst = (volatile void *)&videoBuffer[0];
...
void undrawImageDMA(int row, int col, int width, int height, const u16 *image) {
  ...
  for (int i = 0; i < height; i++){
    ...
    DMA[3].dst = (&videoBuffer[(OFFSET(row + i,col, 240))]);
    DMA[3].cnt = DMA_ON | width |  DMA_DESTINATION_INCREMENT | DMA_SOURCE_INCREMENT | DMA_16;
  }
}
